...
echo $lg_img = "select aid,body from dede_addonarticle where id>$n order by id asc limit $pagen";
...
while($imgarr = mysql_fetch_assoc($lg_img))
{
	...
	echo 'ID:';
	echo $litpic_id= $imgarr['aid'];//id 下次处理从此处开始

	$litpic_url= str_replace('/uploads/','http://img.aimein.com/uploads/',$imgarr['body']);//图片地址
	
	
	echo '缩略图地址：'.$litpic_url."<a href=$litpic_url target=_blank>打开</a>";
	//更新图片地址
	echo $up_sql = "update dede_addonarticle set body=\"$litpic_url\" where aid=\"$litpic_id\"";
